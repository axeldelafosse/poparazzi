...
export const MaterialTabItem = <T extends TabName = any>({
  ...
  const stylez = useAnimatedStyle(() => {
    return {
      ...
      color:
        Math.abs(index - indexDecimal.value) < 0.5 ? activeColor : inactiveColor
    };
  });
  ...
  return (
    ...
    >
      {renderIcon ? (
        <Animated.View>
          <Animated.View style={stylez}>{inactiveIcon}</Animated.View>
          <Animated.View style={[StyleSheet.absoluteFill, stylez]}>
            {activeIcon}
          </Animated.View>
        </Animated.View>
      ) : (
